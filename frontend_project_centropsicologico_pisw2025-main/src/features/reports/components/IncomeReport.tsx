...
export const IncomeReport = ({ data }: Props) => {
  const vigentes = data.filter((r) => r.status !== "Anulado");
  const total = vigentes.reduce((s, r) => s + r.total, 0);
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-muted/50">
            ...
            <th className="p-3 text-left font-bold text-muted-foreground">Forma de pago</th>
      ...
      <div className="flex gap-4 p-4 bg-muted/30 border-t">
        <span className="text-sm font-bold">Total ingresos: <span className="text-green-600 font-extrabold">{money(total)}</span></span>
        <span className="text-sm font-bold">Cantidad: <span className="font-extrabold">{vigentes.length}</span></span>
      </div>
    </div>
  );
};
