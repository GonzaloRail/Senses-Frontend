...
import { getAllAppointmentsPaginatedApi, getAppointmentByIdApi } from "@/features/appointments/api/appointmentsApi";
...
type NotificationType = "24_HOURS" | "2_HOURS" | "CANCELLATION";
type NotificationStatus = "SCHEDULED" | "PROCESSING" | "CANCELLED" | "NOT_AVAILABLE";
...
const typeLabels: Record<NotificationType, string> = { "24_HOURS": "24 horas antes", "2_HOURS": "2 horas antes", CANCELLATION: "Cancelación de cita" };
const statusLabels: Record<NotificationStatus, string> = { SCHEDULED: "Pendiente de envío", PROCESSING: "Procesando", CANCELLED: "Cancelado", NOT_AVAILABLE: "WhatsApp no disponible" };
const statusClasses: Record<NotificationStatus, string> = { SCHEDULED: "bg-amber-50 text-amber-800 border-amber-200", PROCESSING: "bg-blue-50 text-blue-800 border-blue-200", CANCELLED: "bg-slate-100 text-slate-700 border-slate-200", NOT_AVAILABLE: "bg-red-50 text-red-700 border-red-200" };

function formatDate(value: string) {
  return new Intl.DateTimeFormat("es-PE", { dateStyle: "short", timeStyle: "short", timeZone: "America/Lima" }).format(new Date(value));
}

function getSnapshots(): Record<string, ConfirmationSnapshot> {
  try {
    return JSON.parse(localStorage.getItem(confirmationStorageKey) ?? "{}") as Record<string, ConfirmationSnapshot>;
  } catch {
    return {};
  }
}

function saveSnapshots(snapshots: Record<string, ConfirmationSnapshot>) {
  localStorage.setItem(confirmationStorageKey, JSON.stringify(snapshots));
}

function createNotification(appointment: AppointmentViewResponse, phone: string, type: NotificationType, scheduledAt: Date, confirmationStatus: ConfirmationSnapshot["confirmationStatus"]): AppointmentNotification {
  ...
  const confirmationUrl = cancellation ? undefined : `/confirmar-cita/${appointment.id}`;
  const message = cancellation
    ? `Hola, ${patient}.\n\nTe informamos que tu cita del ${formatDate(appointmentDate)} con ${professional}, en ${location}, ha sido cancelada.\n\nPara más información, comunícate con Senses Psicólogos.`
  ...
  return {
    ...
    status: hasPhone ? "SCHEDULED" : "NOT_AVAILABLE",
    confirmationStatus,
    message,
    confirmationUrl,
    detail: hasPhone ? "La entrega y lectura se mostrarán al conectar el proveedor de WhatsApp." : "El paciente no tiene un número de WhatsApp válido registrado.",
  };
}

async function loadNotifications(): Promise<AppointmentNotification[]> {
  const page = await getAllAppointmentsPaginatedApi({ page: 1, take: 100 }) as AppointmentPaginatedResponse;
  const snapshots = getSnapshots();
  const details = await Promise.all(page.appointments.map(async (summary) => {
    const appointment = await getAppointmentByIdApi(summary.id) as AppointmentViewResponse;
    const patient = await getPatientByIdApi({ id: appointment.patient.id }) as PatientContactResponse;
    return { appointment, phone: patient.phoneNumber ?? "" };
  }));

  const now = new Date();
  const notifications = details.flatMap(({ appointment, phone }) => {
    const snapshot: ConfirmationSnapshot = {
      appointmentId: appointment.id,
      patient: `${appointment.patient.firstName} ${appointment.patient.lastName}`,
      professional: `${appointment.user.firstName} ${appointment.user.lastName}`,
      appointmentDate: appointment.startDate,
      location: `${appointment.office.location.name} / ${appointment.office.name}`,
      confirmationStatus: snapshots[appointment.id]?.confirmationStatus ?? "PENDING",
    };
    snapshots[appointment.id] = { ...snapshots[appointment.id], ...snapshot };
    const start = new Date(appointment.startDate);
    if (appointment.status === "CANCELED") return [createNotification(appointment, phone, "CANCELLATION", now, snapshot.confirmationStatus)];
    if (start <= now) return [];
    const first = new Date(start.getTime() - 24 * 60 * 60 * 1000);
    const second = new Date(start.getTime() - 2 * 60 * 60 * 1000);
    return [
      ...(first > now ? [createNotification(appointment, phone, "24_HOURS", first, snapshot.confirmationStatus)] : []),
      ...(second > now ? [createNotification(appointment, phone, "2_HOURS", second, snapshot.confirmationStatus)] : []),
    ];
  });
  saveSnapshots(snapshots);
  return notifications.sort((a, b) => new Date(a.scheduledAt).getTime() - new Date(b.scheduledAt).getTime());
}
...
export const AppointmentNotificationsPage = () => {
  ...
  const [secondReminder, setSecondReminder] = useState(2);
  ...
  const filtered = notifications.filter((notification) => {
    ...
    return (!term || [notification.patient, notification.professional, notification.location, notification.phone].join(" ").toLowerCase().includes(term)) && (!type || notification.type === type) && (!status || notification.status === status);
  });
  ...
  const saveSettings = () => {
    if ((firstEnabled && firstReminder < 1) || (secondEnabled && secondReminder < 1) || (firstEnabled && secondEnabled && firstReminder === secondReminder)) {
      toast.error("Define anticipaciones distintas y mayores que cero.");
      return;
    }
    setSettingsOpen(false);
    toast.success("Configuración guardada localmente. Se conectará al backend cuando el endpoint esté disponible.");
  };
  ...
  <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}><DialogContent><DialogHeader><DialogTitle className="text-senses-primary">Configuración de recordatorios</DialogTitle><DialogDescription>La persistencia global se conectará cuando exista el endpoint de configuración.</DialogDescription></DialogHeader><div className="space-y-5 py-2"><ReminderInput label="Primer recordatorio" enabled={firstEnabled} hours={firstReminder} onEnabledChange={setFirstEnabled} onHoursChange={setFirstReminder} /><ReminderInput label="Segundo recordatorio" enabled={secondEnabled} hours={secondReminder} onEnabledChange={setSecondEnabled} onHoursChange={setSecondReminder} /><div className="rounded-lg bg-senses-secondary/10 p-3 text-sm text-senses-primary"><MessageCircle className="mr-2 inline size-4" />Canal único: WhatsApp.</div></div><DialogFooter><Button variant="outline" onClick={() => setSettingsOpen(false)} className="cursor-pointer">Cancelar</Button><Button onClick={saveSettings} className="cursor-pointer bg-senses-primary hover:bg-senses-primary/90">Guardar configuración</Button></DialogFooter></DialogContent></Dialog>
  </>;
};
...
const ReminderInput = ({ label, enabled, hours, onEnabledChange, onHoursChange }: { label: string; enabled: boolean; hours: number; onEnabledChange: (value: boolean) => void; onHoursChange: (value: number) => void }) => <div className="flex items-center gap-3 rounded-lg border p-3"><input type="checkbox" checked={enabled} onChange={(event) => onEnabledChange(event.target.checked)} className="size-4 accent-[#0B2035]" /><div className="flex-1"><Label>{label}</Label><p className="text-xs text-muted-foreground">Anticipación antes de la cita</p></div><Input type="number" min="1" disabled={!enabled} value={hours} onChange={(event) => onHoursChange(Number(event.target.value))} className="w-20" /><span className="text-sm text-muted-foreground">horas</span></div>;
